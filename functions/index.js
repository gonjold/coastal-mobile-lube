...
// =============================================================================
// TEMPORARY: smoke test for FDACS template visual validation (WO-FDACS-D1-D2)
// REMOVE in step 5 of the same WO. Do not leave deployed.
// =============================================================================
...
exports.smokeTestFdacsTemplate = onRequest(
  {
    region: 'us-east1',
    cors: allowedOrigins,
    memory: '2GiB',
  ...
  async (req, res) => {
    ...
    try {
      // Mock invoice using PRODUCTION schema (serviceName/unitPrice/lineTotal/taxable,
      // subtotal/taxAmount/total, qbTaxAmount/qbTotalAmount).
      const mockInvoice = {
        invoiceNumber: 'CMLT-2026-SMOKE',
        invoiceDate: 'May 7, 2026',
        dueDate: 'May 21, 2026',
        customerName: 'Test FDACS',
        customerAddress: '123 Test Street, Apollo Beach, FL 33572',
        customerPhone: '(813) 555-0123',
        customerEmail: 'testfdacs@example.com',
        vehicleInfo: {
          year: 2019,
          make: 'Toyota',
          model: 'Camry',
        ...
        customerComplaint: 'Customer reports vehicle is overdue for routine oil change and notes a slight pull to the right when braking. Requests tire rotation and brake inspection.',
        lineItems: [
          { serviceName: 'Synthetic Oil Change (5W-30, up to 6 qts)', quantity: 1, unitPrice: 89.99, lineTotal: 89.99, taxable: true },
          { serviceName: 'Oil Filter Replacement', quantity: 1, unitPrice: 12.50, lineTotal: 12.50, taxable: true },
          { serviceName: 'Tire Rotation (4 tires)', quantity: 1, unitPrice: 35.00, lineTotal: 35.00, taxable: false },
          { serviceName: 'Brake Inspection (front and rear)', quantity: 1, unitPrice: 0.00, lineTotal: 0.00, taxable: false },
        ],
      };

      const docType = req.query.docType === 'ESTIMATE' ? 'ESTIMATE' : 'INVOICE';

      // For estimate test, push total over $150 threshold to trigger consent block
      if (docType === 'ESTIMATE') {
        mockInvoice.lineItems.push({ serviceName: 'New Battery (Group 24F, 4-yr warranty)', quantity: 1, unitPrice: 165.00, lineTotal: 165.00, taxable: true });
      }

      if (req.query.addTire === '1') {
        mockInvoice.lineItems.push({ serviceName: 'New Tire (235/65R17 Michelin Defender)', quantity: 1, unitPrice: 189.00, lineTotal: 189.00, taxable: true });
      }
      if (req.query.addBattery === '1' && docType !== 'ESTIMATE') {
        mockInvoice.lineItems.push({ serviceName: 'New Battery (Group 24F)', quantity: 1, unitPrice: 165.00, lineTotal: 165.00, taxable: true });
      }

      // Compute totals from line items (mirrors what the production write path will do).
      const subtotal = mockInvoice.lineItems.reduce((s, li) => s + Number(li.lineTotal || 0), 0);
      const taxableSubtotal = mockInvoice.lineItems
        .filter((li) => li.taxable)
        .reduce((s, li) => s + Number(li.lineTotal || 0), 0);
      const computedTax = Math.round(taxableSubtotal * 0.075 * 100) / 100;

      mockInvoice.subtotal = Math.round(subtotal * 100) / 100;
      mockInvoice.taxAmount = 0; // Firestore mirror — pre-QB-sync state
      mockInvoice.total = mockInvoice.subtotal;

      // Estimates don't have QB fields (no sync until accepted). Invoices do.
      if (docType === 'INVOICE') {
        mockInvoice.qbTaxAmount = computedTax;
        mockInvoice.qbTotalAmount = Math.round((mockInvoice.subtotal + computedTax) * 100) / 100;
        mockInvoice.qbInvoiceId = '999';
        mockInvoice.qbDocNumber = mockInvoice.invoiceNumber;
      }

      const businessSnap = await admin.firestore().doc('settings/business').get();
      if (!businessSnap.exists) {
        res.status(500).json({ error: 'settings/business doc does not exist; run scripts/seed-fdacs-business-settings.js' });
        return;
      }
      const business = businessSnap.data();

      const disclosures = buildDisclosures(mockInvoice, business);
      const html = buildFdacsHtml(mockInvoice, business, disclosures, { documentType: docType });
      ...
      const variantTag = [
        docType.toLowerCase(),
        req.query.addTire === '1' ? 'tire' : '',
        req.query.addBattery === '1' ? 'battery' : '',
      ].filter(Boolean).join('-');
      const storagePath = `fdacs-pdfs/_smoke/${variantTag}-${ts}.pdf`;
      const file = admin.storage().bucket().file(storagePath);
      await file.save(pdfBuffer, {
        contentType: 'application/pdf',
        metadata: { metadata: { generatedBy: 'smokeTestFdacsTemplate', docType, variantTag } },
      });
      ...
      res.status(500).json({ ok: false, error: String(err && err.message || err), stack: err && err.stack });
    }
  }
);
