...
export default function FeesPage() {
  ...
  function handleSaveFee(fee: Fee) {
    ...
    persist(updated, promos);
    setExpandedId(null);
    toast.success(exists ? "Fee updated" : "Fee added");
  }
  ...
  function handleSavePromo(promo: Promo) {
    ...
    persist(fees, updated);
    setExpandedId(null);
    toast.success(exists ? "Promotion updated" : "Promotion added");
  }
  ...
  return (
    <>
      <header className="sticky top-0 z-30 bg-white border-b border-gray-200 px-6 py-3.5 flex items-center justify-between">
        <div>
          <h1 className="text-xl font-bold text-[#0B2040]">Fees &amp; promotions</h1>
          <p className="text-[13px] text-gray-500 mt-0.5">Manage service fees, surcharges, and promotional discounts</p>
        </div>
