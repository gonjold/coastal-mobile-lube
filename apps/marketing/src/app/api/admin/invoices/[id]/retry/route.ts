 ...
 * errored. Reuses the existing onRequest cloud function (FROZEN) by:
 *
 *   1. Verifying the caller is an authenticated admin
 *   2. Reading the invoice doc and reconstructing the payload
 *   3. Minting a custom token for the caller and exchanging it via the
 *      identitytoolkit REST endpoint to get an ID token
 *   4. Calling the cloud function with that token as Bearer
 *
 * Outcome is persisted on the invoice doc:
 ...
 *     + attemptedAt populated. Caller surfaces this in the Fix dialog.
 */
