...
import { type Booking, buildCustomerList } from "./shared";
  ...
  return (
    <>
      ...
      <AdminModalProvider>
        ...
        <div
          className={`min-h-screen bg-background font-sans md:transition-[margin-left] md:duration-200 md:ease-out ${
            sidebarCollapsed ? "md:ml-14" : "md:ml-[240px]"
          }`}
        >
          <main>
...
function CustomerProfileModalWrapper({
  ...
  const customerInvoices = useMemo(() => {
    ...
    return invoices.filter(
      (i) =>
        (i as unknown as { customerName?: string }).customerName?.toLowerCase() ===
        name,
    );
  ...
  const customer: CustomerForPanel | null = matched
    ? {
        ...
        type:
          ((customerBookings[0] as unknown as Record<string, string>)
            ?.customerType as string) || "Residential",
        status: customerBookings.some((b) => b.status === "completed")
          ? "Active"
          : "Lead",
        ...
        communicationPreferences: (
          customerBookings[0] as unknown as Record<string, unknown>
        )?.communicationPreferences as CustomerForPanel["communicationPreferences"],
        notes: (customerBookings[0] as unknown as Record<string, string>)
          ?.notes,
      }
    : null;
